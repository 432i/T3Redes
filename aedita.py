...
class LearningSwitch (object):
  ...
  def _handle_PacketIn (self, event):
    ...
    if not self.transparent: # 2 AKA SE MODIFICA
    ...
    if packet.dst.is_multicast:
      flood() # 3a
        ...
        #packet.src = direccion mac del emisor
        #packet.dst = direccion mac del destino
        #event.port = puerto por donde entra
        #port = puerto por donde sale
        ...
        log.debug("installing flow for %s.%i -> %s.%i" %
                  (packet.src, event.port, packet.dst, port)) #a"""
        #switch X
        if event.port == 8:
